...
def test_real_v2_bootstrap_records_once_and_cache_only_audit_recognizes_it(
    ...
    assert batch.main(
        [*args, "--bootstrap-applied", "--confirm-bootstrap-applied"],
        repair_main=batch.repair.main,
        binding_for_item=batch._offline_binding_for_item,
    ) == 0
    ...
    assert batch.main(args, repair_main=batch.repair.main, binding_for_item=batch._offline_binding_for_item) == 0
    assert json.loads(report.read_text(encoding="utf-8"))["items"][0]["status"] == "already_applied"


def test_external_factory_root_bootstrap_uses_production_cache_key_computation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """An exact external factory root validates caches generated by the real layer."""
    slug, language, video_id = "music_in_the_new_millennium", "en", "tTiah5ncQsw"
    external_work = tmp_path / "external-production-work"
    factory = _write_cache_only_factory(external_work, slug, language)
    narration = factory / "delivery" / language / "narration"
    sidecar = json.loads((factory / "delivery" / language / "narration.inputs.json").read_text(encoding="utf-8"))
    ...
    monkeypatch.setattr("backend.studio.youtube.publishing_package.media_duration", duration)
    monkeypatch.setenv("ELEVENLABS_API_KEY", "local-test-key")
    ...
    class Response:
        ...
        def json(self) -> dict[str, object]:
            # This requester is local: it supplies deterministic timings to
            # the production cache writer and never makes an HTTP request.
            return {"loss": 0.0, "words": self.words}

        def __init__(self, words: list[dict[str, object]]) -> None:
            self.words = words

    def cached_alignment_requester(*_args: object, **kwargs: object) -> Response:
        text = kwargs["data"]["text"]  # type: ignore[index]
        assert isinstance(text, str)
        return Response([
            {"text": word, "start": index * 1.0, "end": index * 1.0 + 0.5, "loss": 0.0}
            for index, word in enumerate(text.split())
        ])

    timing = documentary_timing(factory, language=language, probe=duration)
    vtt = build_aligned_captions(
        factory=factory, hook_audio=narration / "hook.mp3", hook_text=sidecar["transcripts"]["hook"],
        hook_start=timing.hook_start, hook_duration=timing.durations["hook"],
        story_audio=narration / "story.mp3", story_text=sidecar["transcripts"]["story"],
        story_start=timing.story_start, story_duration=timing.durations["story"],
        requester=cached_alignment_requester,
    )
    output = factory / "publishing_repair" / language / "captions.vtt"
    output.parent.mkdir(parents=True)
    output.write_text(vtt, encoding="utf-8")
    ...
    report, ledger = tmp_path / "report.json", tmp_path / "ledger.json"
    args = [
        "--factory-root", str(factory), "--state", str(state), "--report", str(report),
        "--ledger", str(ledger), "--slug", slug, "--language", language,
    ]
    assert batch.main([*args, "--bootstrap-applied", "--confirm-bootstrap-applied"], repair_main=batch.repair.main) == 0
    assert json.loads(ledger.read_text(encoding="utf-8"))["applied"].keys() == {f"{slug}|{language}"}
