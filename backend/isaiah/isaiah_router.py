...
from backend.isaiah.offer_access import OFFER_CODE, compute_complimentary_access, compute_offer_access
...
def spotify_login():
    ...
    scopes = "user-read-private playlist-read-private user-read-email user-read-playback-state user-modify-playback-state" #user-read-playback-state user-modify-playback-state streaming | user-read-private playlist-read-private"
    params = {
...
async def create_checkout_session(access_token: str = Cookie(None)):
    ...
    user = supabase.table("topspot_users") \
        .select("stripe_customer_id, complimentary_access, complimentary_access_expires_at, complimentary_access_reason") \
        .eq("id", user_id) \
        .single() \
        .execute()

    if compute_complimentary_access(user.data):
        raise HTTPException(
            status_code=403,
            detail="Checkout is unavailable while complimentary access is active",
        )
...
async def get_subscription_status(access_token: str = Cookie(None)):
    ...
    if user.data.get("spotify_product") != "premium":
        ...
        """
    complimentary_access = compute_complimentary_access(user.data)
    if complimentary_access:
        return complimentary_access
