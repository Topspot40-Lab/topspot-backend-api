...
async def generate_for_language(language: str) -> None:
    ...
    with Session(engine) as session:
        rows = session.exec(
            ...
            .where(TrackLocale.short_detail_text != None)
            .where(Track.spotify_track_id != None)
            .order_by(TrackLocale.track_id)
        ).all()

        print(f"Found {len(rows)} locale rows with short-detail text")

        for locale, track in rows:
            spotify_id = track.spotify_track_id
            text = (locale.short_detail_text or "").strip()
            ...
            print(f"Generating {language} short detail: {track.track_name} — {spotify_id}")
