...
COLLECTION_NAME = "Gary's Missing Country Favorites"
COLLECTION_SLUG = "garys_missing_country_favorites"
...
def get_or_create_collection(session: Session, category: CollectionCategory) -> Collection:
    ...
    collection = Collection(
        ...
        intro=(
            "A personal country collection of songs and artists Gary kept missing "
            "in TopSpot, from honky-tonk legends and country queens to Texas "
            "troubadours and modern favorites."
        ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--csv",
        default="backend/data/heritage_collections/garys_missing_country_favorites.csv",
    )
    ...
    with Session(engine) as session:
        ...
        for rank, row in enumerate(rows, start=1):
            ...
            if not artist:
                print(f"{rank:02d}. MISSING ARTIST: {artist_name} - {track_name}")
                continue
