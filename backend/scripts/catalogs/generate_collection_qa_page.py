...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--slug", default="railroad_train_songs")
...
def main() -> None:
    ...
    for ranking, track, artist in rows:
        ...
        for lang_code, label in LANGUAGES:
            ...
            if lang_code == "en":
                intro_mp3 = collection_intro_key(collection.slug, ranking.ranking)
            else:
...
<head>
    <meta charset="utf-8">
    <title>QA - {html.escape(collection.name)}</title>
    <style>
        body {{
...
<body>
    <h1>Collection QA</h1>
    <div class="subtitle">{html.escape(collection.name)} — {html.escape(collection.slug)}</div>
...
"""
    ...
    output_path = OUTPUT_DIR / f"collection_qa_{collection.slug}.html"
