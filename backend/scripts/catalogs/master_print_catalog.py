...
FINAL_COVER_ARTWORK = "cover-topspot40-color-concept.png"
...
def render_instructions_page() -> str:
    ...
    return '''<section class="catalog-page instructions"><h1>Using This Catalog</h1><p class="instructions-lead">Choose any program in Program Mode. Program Numbers are optional shortcuts and are not used by Radio Mode.</p><section class="code-guide"><p><strong>N-xxx</strong><span>Nostalgia Program</span></p><p><strong>C-xxx</strong><span>Collection</span></p><p><strong>A-xxx</strong><span>Artist Spotlight</span></p><p><strong>D-xxx</strong><span>Music Docuseries</span></p></section><h2>Two ways to start a program</h2><h3>Browse the menus</h3><ol class="program-steps"><li>Open TopSpot40 and choose Program Mode.</li><li>Choose Nostalgia Programs, Collections Programs, Artist Spotlights, or Music Docuseries.</li><li>Follow the menus to select your program.</li></ol><h3>Use a Program Number shortcut</h3><p class="program-number-shortcut">If you have this catalog, choose Enter Program Number under Program Mode and enter the number printed beside the program.</p><section class="track-request"><h2>Requesting a particular song</h2><p>Give the program number and the song&rsquo;s printed rank. For example, <strong>N-001 / #27</strong> identifies song 27 in Nostalgia program N-001.</p></section></section>'''
...
def render_music_enriches_page() -> str:
    """The first interior page: a warm, large-print welcome for every listener."""
    return '''<section class="catalog-page opening-welcome"><p class="opening-eyebrow">TOPSPOT40</p><h1>Music Enriches Every Stage of Life</h1><p class="opening-lead">A familiar song can bring back a place, a person, or a moment&mdash;and create a new one when it is shared. TopSpot40 makes room for listening, remembering, discovering, and talking about the music that stays with us.</p><p>Explore on your own, with family and friends, or with a group. Each program brings together carefully chosen music, welcoming introductions, artist stories, and musical history.</p><section class="founder-note"><h2>Why We Created TopSpot40</h2><p>Gary grew up listening to radio stories about songs and artists, from farm mornings to school-bus rides and <em>American Top 40</em>. Patty&rsquo;s love of language, culture, and music helped inspire a service that welcomes listeners in English, Spanish, and Brazilian Portuguese.</p><p class="founder-signature">&mdash; Gary Steele, Founder of TopSpot40</p><blockquote><strong>Patty&rsquo;s Rule</strong><br>If a feature is too complicated to enjoy, it probably needs to be simplified.</blockquote></section></section>'''
...
def render_closing_conclusion() -> str:
    ...
    return f'''<section class="catalog-page closing-conclusion"><div class="back-cover-frame"><div class="closing-copy"><h1>Every Song Holds a Story</h1><p>A song can carry us across decades&mdash;to a childhood home, a school dance, a family celebration, or an evening spent with someone we loved. Music reconnects us with people, places, and moments that remain part of our lives.</p><p>TopSpot40 brings those connections to life through carefully chosen programs, narrated introductions, artist stories, and musical history. Listen to a complete program, explore a favorite artist or collection, or discover something entirely new.</p><p>Whether you listen alone, with family and friends, or as part of a group, we hope TopSpot40 helps you remember something meaningful, share a story, and enjoy the music that connects us.</p><p class="closing-line">Music, memories, and moments&mdash;shared one song at a time.</p><img class="closing-brand-icon" src="{html.escape(logo, quote=True)}" alt="TopSpot40 — Old Dog, New Tracks"><p class="closing-thanks">Thank You for Listening</p><p class="closing-signature">TopSpot40 &mdash; Music, Memories &amp; Moments</p></div></div></section>'''
...
def render_html(programs: list[Program], *, generated_on: date | None = None, nostalgia_qr: Callable[[Program], str | None] | None = None, collection_qr: Callable[[Program], str | None] | None = None, artist_qr_url: str | None = None, artist_rows: list[dict[str, Any]] | None = None, docuseries_groups: list[DocuseriesGroup] | None = None, cover_artwork: str = FINAL_COVER_ARTWORK) -> str:
    generated = (generated_on or date.today()).isoformat()
    ...
    pages = [
        '<section class="catalog-page cover"><img src="' + ARTWORK_URL + '/' + html.escape(cover_artwork, quote=True) + '" alt="TopSpot40 — Music, Memories &amp; Moments"></section>',
        render_music_enriches_page(),
    ...
    major_sections = [("Quick Reference Guide", 4), ("Discovery Guide", first_page_containing('class="catalog-page discovery'))]
    if nostalgia:
...
.opening-welcome {{ background: linear-gradient(120deg, #f7f1e2 0%, #fffdf8 62%, #e7eef1 100%); padding-top: .72in; }} .opening-eyebrow {{ margin: 0 0 .12in; color: #a37a24; font-size: 12pt; font-weight: bold; letter-spacing: .15em; }} .opening-welcome h1 {{ margin: 0 0 .26in; max-width: 8.7in; color: #173a5e; font-size: 33pt; line-height: 1.08; }} .opening-welcome > p {{ max-width: 8.8in; margin: 0 0 .2in; font-family: Georgia, serif; font-size: 15pt; line-height: 1.48; }} .opening-welcome .opening-lead {{ font-size: 18pt; line-height: 1.42; }} .opening-welcome .founder-note {{ max-width: 8.9in; margin-top: .28in; }} .opening-welcome .founder-note h2 {{ color: #173a5e; font-size: 18pt; }} .opening-welcome .founder-note p {{ max-width: none; font-size: 12.5pt; line-height: 1.32; }}
.instructions {{ background: #fcfbf8; padding-top: .56in; }} .instructions h1 {{ margin: 0 0 .16in; color: #173a5e; font-size: 33pt; }} .instructions h2 {{ margin: .24in 0 .11in; color: #173a5e; font-size: 21pt; }} .instructions .instructions-lead {{ max-width: 8.8in; margin: 0 0 .22in; font-size: 16pt; line-height: 1.38; }} .code-guide {{ display: grid; grid-template-columns: 1fr 1fr; max-width: 8.5in; gap: .1in .34in; }} .code-guide p {{ display: flex; align-items: baseline; gap: .16in; margin: 0; padding: .12in .15in; border-left: 5px solid #b48a32; background: white; font-size: 15pt; }} .code-guide strong {{ min-width: .7in; color: #173a5e; }} .program-steps {{ max-width: 8.7in; margin: 0; padding-left: .32in; font-size: 16pt; line-height: 1.55; }} .program-steps li {{ padding-left: .06in; }} .track-request {{ max-width: 8.7in; margin-top: .22in; padding: .14in .18in; border-top: 2px solid #b48a32; }} .track-request h2 {{ margin: 0 0 .05in; font-size: 18pt; }} .track-request p {{ margin: 0; font-size: 14pt; line-height: 1.35; }}
.closing-conclusion {{ padding: .34in .42in; background: #f5f0e3; }} .closing-conclusion .back-cover-frame {{ display: block; border: 2px solid #173a5e; outline: 2px solid #b48a32; outline-offset: 4px; background: #fffdf8; }} .closing-conclusion .closing-copy {{ max-width: 8.65in; margin: 0 auto; padding: .44in .55in .3in; text-align: center; }} .closing-conclusion h1 {{ margin: 0 0 .25in; color: #173a5e; font-size: 26pt; line-height: 1.1; }} .closing-conclusion p {{ margin: 0 0 .18in; color: #292929; font-family: Georgia, serif; font-size: 14.5pt; line-height: 1.48; text-align: left; }} .closing-conclusion .closing-line {{ margin: .26in 0 .18in; color: #173a5e; font-size: 15pt; font-style: italic; font-weight: bold; text-align: center; }} .closing-conclusion .closing-brand-icon {{ display: block; width: 2.5in; height: auto; margin: .05in auto .12in; }} .closing-conclusion .closing-thanks {{ margin: 0 0 .04in; color: #173a5e; font-family: Arial, sans-serif; font-size: 16pt; font-style: normal; font-weight: bold; text-align: center; }} .closing-conclusion .closing-signature {{ margin: 0; color: #665c4d; font-family: Arial, sans-serif; font-size: 13.5pt; line-height: 1.25; text-align: center; }}
...
def generate(output: Path, *, nostalgia_url_template: str | None = None, collection_url_template: str | None = None, artist_qr_url: str | None = None, quick_reference_output: Path | None = None, cover_artwork: str = FINAL_COVER_ARTWORK) -> Path:
    ...
    output.write_text(render_html(programs, artist_rows=artist_rows, docuseries_groups=docuseries_groups, cover_artwork=cover_artwork), encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--cover-artwork", default=FINAL_COVER_ARTWORK)
    args = parser.parse_args()
    print(f"Generated: {generate(args.output, nostalgia_url_template=args.nostalgia_url_template, collection_url_template=args.collection_url_template, artist_qr_url=args.artist_qr_url, quick_reference_output=args.quick_reference_output, cover_artwork=args.cover_artwork)}")
